...
import { createSubtopic, updateSubtopic, deleteSubtopic } from '@/app/(dashboard)/admin/subtopics/actions';
...
export function AdminSubtopicsClient({
  selectedLevel,
  chapters,
  errorMessage,
}: {
  ...
  const addFormRefs = useRef<Map<string, HTMLDetailsElement>>(new Map());
  ...
  async function handleCreate(formData: FormData) {
    ...
    await createSubtopic(formData);
    const ref = addFormRefs.current.get(chapterId);
              ...
              {chapters.map((chapter) => (
                <div key={chapter.id} className="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
                  <div className="flex items-center justify-between bg-zinc-50 px-4 py-3">
                    ...
                    <span className="text-xs text-zinc-500">{chapter.subtopics.length} subtopics</span>
                  </div>
                  ...
                  <div className="divide-y divide-zinc-100">
                          ...
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div className="flex shrink-0 items-center gap-3">
                              ...
                              <form action={handleDelete}>
                                ...
                                <button
                                  type="submit"
                                  onClick={(e) => { if (!confirm('Delete this subtopic?')) e.preventDefault(); }}
                    ...
                    <div className="px-4 py-3">
                      <details
                        className="group"
                        ref={(el) => {
                          if (el) addFormRefs.current.set(chapter.id, el);
                          else addFormRefs.current.delete(chapter.id);
                        }}
                      >
