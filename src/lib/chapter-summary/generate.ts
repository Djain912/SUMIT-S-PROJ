...
/** Generates (but does not persist) a Quick Revision Sheet for one chapter. */
export async function generateChapterSummary(chapterId: string) {
  ...
  if (!chapter) throw new SummaryGenerationError('Chapter not found', 404);
  ...
  if (!sourceText) {
    throw new SummaryGenerationError('This chapter has no published notes to summarize yet.', 400);
  }
  ...
  try {
    ...
    throw new SummaryGenerationError('AI returned malformed output, please retry.', 502);
  }
